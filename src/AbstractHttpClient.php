...
abstract class AbstractHttpClient {
     ...
     * @return \WebResponse Response object
     */
    public function executeHttpRequest($path, $contentType, $accept) {
        try {
            ...
            return new WebResponse($this->curlHandle, $response);
        } catch (Exception $ex) {
     ...
     * lifecycle defined as open, prepare, write, read. Each of these methods in
     * turn delegates to the {@link RequestHandler} associated with this client.
     * @param WebRequest $httpRequest
     * @return \WebResponse
     */
    public function exec($httpRequest) {
        $httpResponse = NULL;
        try {
            if ($httpRequest != null && $httpRequest instanceof WebRequest) {
                // Not yet connected
                $this->connected = FALSE;
                ...
                if ($response === FALSE) {
                    ...
                    $httpResponse = new WebResponse($this->curlHandle, $response);
                }
            }
        } catch (Exception $ex) {
     ...
     * @param string $accept
     * @internal param Handle $cURL $curlHandle
     */
    ...
    public function execute($httpRequest) {
        ...
        if ($httpRequest instanceof WebRequest) {
     ...
     * @param $curlHandle
     * @return type
     * @internal param Handle $cURL $curHandle
     */
     ...
     * @return WebResponse response object
     * @internal param string $accept The Response MIME
     */
    ...
    protected function isTimeoutException() {
        ...
        $error_no = curl_errno($this->curlHandle);
        if ($error_no === CURLE_OPERATION_TIMEOUTED) {
            if ($this->logger->isLoggingEnabled()) {
                $this->logger->log("cURL error ($error_no): \n Operation timeout. "
     ...
     * @param array $assoc
     * @param array $files
     * @return string The payload body
     */
